...
async function startServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    const { prompt, region = 'South Kerala Coast', role = 'fisherman', preferredLanguage, history, deviceLocation } = req.body;
    ...
    try {
      const result = await handleChat({ query: prompt, region, role, preferredLanguage, history, deviceLocation });
